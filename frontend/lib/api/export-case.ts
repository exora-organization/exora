...
export const apiExportCase = {
  list: async (): Promise<ApiResponse<PaginatedResponse<ExportCaseListItem>>> => {
    return apiClient<ApiResponse<PaginatedResponse<ExportCaseListItem>>>("/own-export-cases", {
      method: "GET",
    });
  },

  get: async (caseId: string): Promise<ApiResponse<ExportCaseResponse>> => {
    return apiClient<ApiResponse<ExportCaseResponse>>(`/own-export-cases/${caseId}`, {
      method: "GET",
    });
  },

  create: async (data: CreateExportCaseRequest): Promise<ApiResponse<ExportCaseResponse>> => {
    return apiClient<ApiResponse<ExportCaseResponse>>("/own-export-cases", {
      method: "POST",
  ...
  delete: async (caseId: string): Promise<ApiResponse<void>> => {
    return apiClient<ApiResponse<void>>(`/own-export-cases/${caseId}`, {
      method: "DELETE",
    });
  },
};
