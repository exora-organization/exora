...
export const apiUsers = {
  ...
  listUsers: async (): Promise<ApiResponse<{ items: UserProfile[] }>> => {
    return apiClient<ApiResponse<{ items: UserProfile[] }>>("/admin-users", {
      method: "GET",
    });
  },

  updateUser: async (userId: string, data: { status?: "active" | "disabled" }): Promise<ApiResponse<UserProfile>> => {
    return apiClient<ApiResponse<UserProfile>>(`/admin-users/${userId}`, {
      method: "PATCH",
  ...
  changeRole: async (userId: string, role: string): Promise<ApiResponse<UserProfile>> => {
    return apiClient<ApiResponse<UserProfile>>(`/admin-users/${userId}/role`, {
      method: "PATCH",
  ...
  deleteUser: async (userId: string): Promise<ApiResponse<void>> => {
    return apiClient<ApiResponse<void>>(`/admin-users/${userId}`, {
      method: "DELETE",
    });
  },
};
