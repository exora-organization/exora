...
- [4. Functional Requirements](#4-functional-requirements)
  ...
  - [Export Engine Requirements (Retained from v2.1)](#export-engine-requirements-retained-from-v21)
...
- [8. User Stories](#8-user-stories)
  ...
  - [8.2 Team Management](#82-team-management)
  - [8.3 Multi Tenant Access](#83-multi-tenant-access)
  - [8.4 Export Case Management](#84-export-case-management)
  - [8.5 Pricing Calculation](#85-pricing-calculation)
  - [8.6 Financial Analysis](#86-financial-analysis)
  - [8.7 Scenario Analysis](#87-scenario-analysis)
  - [8.8 Risk Assessment & Feasibility](#88-risk-assessment--feasibility)
  - [8.9 AI Decision Advisor](#89-ai-decision-advisor)
  - [8.10 Dashboard & Documents](#810-dashboard--documents)
- [9. Use Cases](#9-use-cases)
- [10. Entity Relationship Diagram (ERD)](#10-entity-relationship-diagram-erd)
- [11. Database Design (Cloud Firestore)](#11-database-design-cloud-firestore)
- [12. System Architecture](#12-system-architecture)
- [13. RBAC Middleware Design](#13-rbac-middleware-design)
- [14. Dashboard Design](#14-dashboard-design)
- [15. Export Feasibility Scoring Model](#15-export-feasibility-scoring-model)
- [16. Export Costing & Financial Analysis Formulas](#16-export-costing--financial-analysis-formulas)
...
- Backend-managed RBAC with five roles: guest, company_owner, export_manager, finance_staff, admin
...
- Approve application → user becomes company_owner, company status = approved
...
Roles: guest, company_owner, export_manager, finance_staff, admin.
...
Authorized company users (company_owner, export_manager, finance_staff) and Admin may:
- Generate AI recommendations for export cases within their scope
- View AI recommendations

Guest users shall not access the AI Advisor.

**FR-009a AI Advisor — Role-Scoped Content & RAG Behavior**

The AI Decision Advisor shall tailor generated content, RAG retrieval scope, and associated report access per role as follows:

| Role | Advisor Mode | Content Focus | RAG Retrieval Scope | Linked Report(s) |
|---|---|---|---|---|
| Admin | Oversight & Troubleshooting | System health only: RAG retrieval latency/success rate, knowledge base coverage per destination market, anonymized/aggregated recommendation sampling for quality spot-checks, error/anomaly logs. Admin shall NOT receive business-decision recommendations tied to a specific export case. | Metadata-level only (query performance, confidence scores, coverage); drill-down into case-level content permitted only for debugging a reported issue. | None (internal system health view; not a generated business PDF) |
| Company Owner | Portfolio Feasibility | Go/No-Go feasibility summary aggregated across all company export cases; risk rollup (country, payment term, profitability) across the company portfolio. | Company-wide — all destination markets touched by any case within the company. | Export Feasibility Report (FR-022) |
| Export Manager | Pricing & Scenario Guidance | Incoterm scenario guidance (EXW/FOB/CFR/CIF), pricing sensitivity to exchange rate and margin changes, recommendations scoped to the single export case currently open. | Case-level — limited to the destination market of the case currently open. | Quotation (FR-020), Proforma Invoice (FR-021) |
| Finance Staff | Profitability & Financial Risk | Profitability guidance (margin vs. target flags, cost adjustment suggestions), payment term risk commentary, financial risk narrative. | Case-level — limited to payment term and trade-finance references relevant to the case under review. | Cost Breakdown Report (FR-023) |
| Guest | Not applicable | No access. | No access. | No access. |

Acceptance criteria for FR-009a:
- Admin's AI Advisor view shall never surface case-specific business recommendations as its primary content; case-level detail is accessible only in an explicit debug/troubleshooting drill-down.
- Company Owner's RAG queries shall span all markets present across the company's export cases.
- Export Manager and Finance Staff RAG queries shall be scoped to the single case in context; cross-case retrieval is out of scope for these roles.
- Each role's Advisor output shall link only to the report type(s) listed above; generating a report type not listed for a role requires a separate, explicitly approved user story.
...
Granted when: Admin approves company application (role changes from guest to company_owner).
...
| US-004 | High | As an Admin, I want to approve company applications. | On approve: company status = approved, applicant role = company_owner. | FR-003 |
...
| Story ID | Priority | User Story | Acceptance Criteria | Linked FR |
|---|---|---|---|---|
| US-007 | High | As a Company Owner, I want to invite Export Managers. | Invitation created with role export_manager; email/link sent; expires per NFR-014. | FR-005 |
| US-008 | High | As a Company Owner, I want to invite Finance Staff. | Invitation created with role finance_staff; email/link sent. | FR-005 |
...
| Story ID | Priority | User Story | Acceptance Criteria | Linked FR |
|---|---|---|---|---|
| US-016 | High | As an Export Manager, I want to see EXW selling price based on cost inputs and target margin. | EXW calculated per Section 16 formulas. | FR-011 |
...
4. System updates applicant role = company_owner, links companyId
...
**Actor:** Company Owner
**Precondition:** User has role = company_owner and approved company.
**Flow:**
1. Owner enters invitee email and selects role (export_manager or finance_staff)
...
**Precondition:** User has role = export_manager and valid companyId.
**Flow:**
...
| email | string | |
| displayName | string | |
| role | enum | guest, company_owner, export_manager, finance_staff, admin |
...
| role | enum | export_manager, finance_staff |
| companyId | FK | |
...
| export_cases | Company | caseId, companyId, createdBy |
| cost_data | Company | caseId, companyId |
| pricing_results | Company | caseId, companyId |
| financial_analysis | Company | caseId, companyId |
| scenarios | Company | caseId, companyId |
| risk_assessments | Company | caseId, companyId |
| advisor_recommendations | Company | caseId, companyId |
| documents | Company | caseId, companyId |
| audit_logs | Platform | action, actorId, targetId, timestamp |
...
Example:
```go
RequireRoles("admin", "company_owner")
```
...
Responsibilities:
- Log admin actions to audit_logs collection
...
**AI Advisor Module (Oversight & Troubleshooting — see FR-009a)**
- RAG retrieval health (latency, success/fail rate)
- Knowledge base coverage per destination market
- Anonymized/aggregated recommendation sampling
- Error/anomaly log
...
*(Unchanged from v2.1 — see Section 16 formulas below)*
...
| AC-002 | Guest onboarding flow works: register → apply → pending → approve → company_owner |
...
| AC-012 | Admin actions logged in audit_logs |
...
sequenceDiagram
    ...
    API->>FS: User role=company_owner, companyId set
    API-->>A: Approved
```
